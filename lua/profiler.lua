...
local function on_hook(event, func_info_id, source)
    func_info_map = func_info_map or {}
	...
	elseif event == "return" then --or event == 'tail return' then
        local now = get_time()
    ...
    --if func_info.db_info.short_src and func_info.db_info.linedefined > 0 then
    --    --if func_info.db_info.short_src == 'UI/GameUI/MainGame/Race/RaceShopUI' or func_info.db_info.short_src == 'Logic/Start' then
    --    print('{0}:{1}, {2}, {3}', func_info.db_info.short_src, func_info.db_info.linedefined, event, debug.traceback())
    --    --end
    --
    --end
end
