SwornUI = class('SwornUI', BaseWindow)

function SwornUI:Create()
...
function SwornUI:OnPressESCKey()
    if self.close_btn then
        self.close_btn.onClick:Invoke()
    end
end

function SwornUI:Init()
    ...
    if self.confirm_btn then
        self:AddButtonClickListener(self.confirm_btn,function()
			if not self.choseid then
                SystemUICall:GetInstance():Toast('请先选择一名角色进行结义')
                return
            end
            if self.netData and self.netData.iType then
                ...
                RemoveWindowImmediately("SwornUI",false)
            end
		end)
    end
    ...
    self.netData = {}
    self.title.text = "结义"
    self.bottom_text.text = "请选择一名好感度达到100的角色进行结义"
end

function SwornUI:RefreshUI(info)
    self.scrollRect:AddListener(function(...) self:UpdateItem(...) end)
    self:InitList()
    self.netData = info
end

function SwornUI:InitList(info)
    local teammates = RoleDataManager:GetInstance():GetRoleTeammates()
    self.list = {}
    for i,v in pairs(teammates) do
        if self:CanSworn(v) and not self:IsCloseSwornAndMarry() then
    ...
    if #self.list == 0 then
        SystemUICall:GetInstance():Toast('队伍中无可结义角色')
    end
end

function SwornUI:UpdateList()
    self.scrollRect.totalCount = #self.list
	self.scrollRect:RefillCells()
end

function SwornUI:UpdateItem(transform,iIndex)
...
-- 筛选机制
function SwornUI:CanSworn(roleID)
...
-- 判断是否孩童，不单判断主角的徒弟
function SwornUI:IfChild(roleID)
...
-- 判断结婚结义是否被关闭
function SwornUI:IsCloseSwornAndMarry()
...
function SwornUI:OnEnable()
    
end

function SwornUI:OnDisable()

end

function SwornUI:OnDestroy()
end
