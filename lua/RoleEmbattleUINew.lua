RoleEmbattleUINew = class("RoleEmbattleUINew",BaseWindow)
...
function RoleEmbattleUINew:ctor(obj)
...
-- function RoleEmbattleUINew:Create()
-- 	local obj = LoadPrefabAndInit("Role/RoleEmbattleUINew",UI_UILayer,true)
-- 	if obj then
-- 		self:SetGameObject(obj)
-- 	end
-- end

function RoleEmbattleUINew:Init()
	self.objBackButton = self:FindChildComponent(self._gameObject,"Button_back","Button")
...
function RoleEmbattleUINew:Update()
	if self.bUpdateItemPos then
		...
		or ((not g_IS_WINDOWS) and (DRCSRef.Input.touchCount > 0)) then
			--对此UI进行了0.64的缩放 这里要对位置除以0.64的补偿和偏移位置的补偿
			--115 134 分别是跟随鼠标移动的物体的宽长
			--如果无法理解 可以将UI拉伸到原比例　直接self.comRole_Head_Embattle.localPosition = GetTouchUIPos()即可
			self.comRole_Head_Embattle.localPosition = GetTouchUIPos() / 0.64 + DRCSRef.Vec3(115 * 0.36,134 * 0.18,0)
		else
			self:HideChooseItem()
...
function RoleEmbattleUINew:OnPressESCKey()
    if self.objBackButton then
	    self.objBackButton.onClick:Invoke()
    end
end

function RoleEmbattleUINew:RefreshUI(embattleInfo)
	self.bOpenByWheelWar = false
...
function RoleEmbattleUINew:InitCommonEmbattleInfo(eType)
	if not eType then
		return
	end
...
function RoleEmbattleUINew:CheckCommonEmbattleSubmit()
	...
	RemoveWindowImmediately("RoleEmbattleUINew")
	return true
end

function RoleEmbattleUINew:CheckCommonEmbattleCancel()
	if not self.eCommomEmbattleType then
		return
	end
...
function RoleEmbattleUINew:ShowPetList(bshow)
	if bshow then
...
function RoleEmbattleUINew:OnClickAssistTips()
...
function RoleEmbattleUINew:OnClickStartWheelWarButton(bStart)
	if bStart  then
	...
	self:CloseUI() 
    local data =  EncodeSendSeGC_Click_Battle_WheelWar_Result(bStart == true and 1 or 0)
    local iSize = data:GetCurPos()
    NetGameCmdSend(SGC_CLICK_BATTLE_WHEEL_WAR_RESULT, iSize, data)
end

function RoleEmbattleUINew:OnClickCommonStartButton()
...
function RoleEmbattleUINew:CheckRoleEnterClanEliminate(roleID, cantChooseClan, isEvilEliminate)
...
function RoleEmbattleUINew:JudgeWheelWarCondition()
	--合法性判断 是否每场都有单位 且不重复
	-- TODO : 没有 不可加入的门派角色
...
function RoleEmbattleUINew:SendUpdateRoleEmbattleData()
...
-- 更新布阵按钮
function RoleEmbattleUINew:InitEmbattleButtons()
...
function RoleEmbattleUINew:RefreshToggle()
	if self.iGameState == -1 then --平台布阵
		...
		for k, objToggle in pairs(self.schemeID2ToggleNode) do
			...
			if self.bOpenByWheelWar then
				if data.EmbattleType ~= EmBattleSchemeType.EBST_Normal then
					bShow  = true
...
function RoleEmbattleUINew:DealAssistEmbattle()
...
--更新当前布阵信息
function RoleEmbattleUINew:UpdateEmbattleInfo(iToggleID,iRound)
...
function RoleEmbattleUINew:UpdateItemAll()
...
function RoleEmbattleUINew:Anim_MoveEmbattle(bIsHelp)
	local timer = 0.5
	if bIsHelp then
...
function RoleEmbattleUINew:UpdateWheelWarButton()
	--更新车轮战按钮
	if self.bOpenByWheelWar then
...
function RoleEmbattleUINew:isInBattleTeam(iRoleID)
...
function RoleEmbattleUINew:IsSubRole(iRoleID)
...
function RoleEmbattleUINew:isInTurnTeam(iRoleID)
...
function RoleEmbattleUINew:getCurSubRoleDesc(iRoleID)
	-- TODO 暂时这样
	if self.iGameState == -1 then
		return "";
    end
...
function RoleEmbattleUINew:UpdateItem(transform,iIndex)
	...
	local objCreateFaceParent = self:FindChild(transform.gameObject, "CreateHead")
	local iStoryId = (GetGameState() == -1) and 0 or GetCurScriptID()
	local objCreateFace = self:FindChild(transform.gameObject, "CreateHead/Create_Head(Clone)")
	...
	if iRoleTypeID == iMainRoleCreateRoleID and CreateFaceManager:GetInstance():GetFaceDataByStoryIDAndRoleId(iStoryId, iRoleTypeID) then
		...
		if objCreateFace then
			objCreateFace = CreateFaceManager:GetInstance():GetCreateFaceHeadImage(iStoryId, iRoleTypeID, objCreateFaceParent, objCreateFace)
			objCreateFace:SetActive(true)
		else
			objCreateFace = CreateFaceManager:GetInstance():GetCreateFaceHeadImage(iStoryId, iRoleTypeID, objCreateFaceParent)
		end
	else
		if objCreateFace then
	...
	if comImageBtn then
		...
		self:AddButtonClickListener(comImageBtn, function()
			...
			elseif bIsHelpEmbattle then
				...
				if bInTeam then
					...
					if iCount > 1 then
						...
						if strState == "助战中" then
							SystemUICall:GetInstance():Toast("该角色在助阵,无法上阵")
						elseif strState == "替补中" then
							SystemUICall:GetInstance():Toast("该角色在替补,无法上阵")
						end
					else
						if strState == "助战中" then
							SystemUICall:GetInstance():Toast("该角色在助阵,无法上阵")
			...
			if IsWindowOpen("MazeUI") then
				local mazeWindow = GetUIWindow("MazeUI")
				if mazeWindow then
					mazeWindow:UpdateMaze()
				end
			end
		end);
	end
...
function RoleEmbattleUINew:OnClickWatchBtn(roleID)
	OpenWindowImmediately("ObserveUI", {['roleID'] = roleID})
	local _callback = function()
		local roleEmbattleUI = GetUIWindow('RoleEmbattleUINew')
...
function RoleEmbattleUINew:UpdatePetItem(transform,iIndex)
...
function RoleEmbattleUINew:RefreshItem(iRoleID)
	local iIndex = -1
...
function RoleEmbattleUINew:ShowChooseItem()
	self.comRole_Head_Embattle.gameObject:SetActive(true)
	self.bUpdateItemPos = true
end

function RoleEmbattleUINew:HideChooseItem()
	self.comRole_Head_Embattle.gameObject:SetActive(false)
	self.bUpdateItemPos = false
end

function RoleEmbattleUINew:ReSetSubConfig()
	self.OldSelectEmbattleRole = 0
	self.curSelectEmbattleRole = 0
	self.iSelectRoleID = nil
end

function RoleEmbattleUINew:SelectEmbattleToggle(embattleTypeID,callback)
...
function RoleEmbattleUINew:ResetAllRole(iToggleID,iRound)
...
function RoleEmbattleUINew:SelectRoundToggle(iRoundIndex)
	self:UpdateEmbattleInfo(self.iCurToggleID,iRoundIndex)
end

function RoleEmbattleUINew:OnPointDownItem(iRoleID)
	local roleDataMgr = RoleDataManager:GetInstance()
	if self.setSubRoleTab then
		--处理替补操作
	...
	--捏脸50
	local objCG = self:FindChild(nodeHead,"head")
	local objCreateCGParent = self:FindChild(nodeHead,"CreateHead")
	local objCreateFace = self:FindChild(nodeHead,"CreateHead/Create_Head(Clone)")
	local iStoryId = (GetGameState() == -1) and 0 or GetCurScriptID()
	local iMainRoleTypeID = RoleDataManager:GetInstance():GetMainRoleTypeID()
	local iMainRoleCreateRoleID = PlayerSetDataManager:GetInstance():GetCreatRoleID()
	if iRoleTypeID == iMainRoleTypeID then 
	...
	if iRoleTypeID == iMainRoleCreateRoleID and CreateFaceManager:GetInstance():GetFaceDataByStoryIDAndRoleId(iStoryId, iRoleTypeID) then
		...
		if objCreateFace then
			objCreateFace = CreateFaceManager:GetInstance():GetCreateFaceHeadImage(iStoryId, iRoleTypeID, objCreateCGParent, objCreateFace)
			objCreateFace:SetActive(true)
		else
			objCreateFace = CreateFaceManager:GetInstance():GetCreateFaceHeadImage(iStoryId, iRoleTypeID, objCreateCGParent)
		end
	else
...
function RoleEmbattleUINew:AddSubList(uiRoleID)
...
function RoleEmbattleUINew:DelSubList(uiRoleID)
	if self.subList[uiRoleID] then
...
function RoleEmbattleUINew:SetSubRoleShow(uiRoleID, bBattle)
	if bBattle then
...
function RoleEmbattleUINew:RemoveSubRole(uiRoleID)
	if uiRoleID > 0 then
...
function RoleEmbattleUINew:SetGridActor(x,y,iRoleID,bHelp,bpet,bSub)
	local roleData = nil
	...
	if bHelp then
		...
		if self.iCurToggleID == EmBattleSchemeType.EBST_Help or self.iCurToggleID == EmBattleSchemeType.EBST_ArenaHelp then
			objCurActor.transform.position =  DRCSRef.Vec3(position.x - EMBATTLE_OFFSET_X * 0.64 ,position.y - EMBATTLE_OFFSET_Y * 0.64,position.z - EMBATTLE_OFFSET_Z * 0.64);
		else
			objCurActor.transform.position = position
		end
...
function RoleEmbattleUINew:FreeGridActor(x,y)
...
function RoleEmbattleUINew:GetGridRoleID(x,y)
...
function RoleEmbattleUINew:GetInEmbattleType(iRoleID,iRound)
	if iRound == nil then
...
function RoleEmbattleUINew:DelAssistEmbattle(iRoleID)
	if self.iCurToggleID == EmBattleSchemeType.EBST_Help or
	self.iCurToggleID == EmBattleSchemeType.EBST_ArenaHelp then
		return
	end
...
--获取上阵最大人数
function RoleEmbattleUINew:GetMaxTeamCout()
	local iMaxCount = 5;
	if self.iGameState == -1 then
...
function RoleEmbattleUINew:SetRole2Grid(x,y,iRoleID)
...
-- 添加助战成员
function RoleEmbattleUINew:SetAssistRole2Grid(iRoleID, bInTeam, bPet ,bIsHelpRole)
...
function RoleEmbattleUINew:ResetPet(typeid)
	local embattleData = self:GetHelpData()
	for index, data in ipairs(embattleData) do
...
function RoleEmbattleUINew:ResetRole(iRoleID,iRound,bFreeGridActor)
	if iRound == nil then
		iRound = self.iCurRound
	end
...
function RoleEmbattleUINew:UpdateToggleText()
...
function RoleEmbattleUINew:canEmbattle(roleID)
	if not roleID then
		return true
	end
...
function RoleEmbattleUINew:OnDrag(obj,bInParent)
	if self.setSubRoleTab then
		self.iSelectRoleID = nil
		return
	end
...
function RoleEmbattleUINew:OnBeginDrag(obj,bInParent)
	if self.setSubRoleTab then
		self.iSelectRoleID = nil
...
function RoleEmbattleUINew:OnEndDrag(obj,bInParent,bInTeam)
	if self.setSubRoleTab then
		self.iSelectRoleID = nil
		return
	end
...
function RoleEmbattleUINew:OnClickDownGrid(obj,bInParent)
	local name = tonumber(obj.name)
	if name then
		local w = SSD_BATTLE_GRID_WIDTH / 2
		local f = math.floor((name - 1) / w)
...
function RoleEmbattleUINew:SetInitSelectSub()
...
function RoleEmbattleUINew:SetSelectGridColor(obj, bShow)
	if bShow then
		if IsValidObj(obj) then
			local name = tonumber(obj.name)
			if name then
...
function RoleEmbattleUINew:CloseUI()
	self:ReSetSubConfig()
	self.objActor_Node:SetActive(false)
...
function RoleEmbattleUINew:SetUpdateFlag(iID,iRound)
...
function RoleEmbattleUINew:CheckArenaPetsSafe()
	-- 更换宠物后从新布阵错误
...
function RoleEmbattleUINew:OnDisable()
...
function RoleEmbattleUINew:OnEnable()
...
function RoleEmbattleUINew:SortEmbattleRole()
	self.aiTeamRoleID = self:GetTeamRoles()
end


function RoleEmbattleUINew:OnDestroy()
	self.SpineRoleUI:Close()
end

function RoleEmbattleUINew:SetType(type)
	self.type = type;
end

function RoleEmbattleUINew:OnRefUIByPlayerSetting()
...
function RoleEmbattleUINew:OnClickSingleTog()
	self.bIsSingle = true;
	self:SelectEmbattleToggle(EmBattleSchemeType.EBST_Single);
end

function RoleEmbattleUINew:OnClickTeamTog()
	self.bIsSingle = false;
	self:SelectEmbattleToggle(EmBattleSchemeType.EBST_Team);
end

function RoleEmbattleUINew:OnClickHelpTog()
	self.bIsSingle = true;
	self:SelectEmbattleToggle(EmBattleSchemeType.EBST_ArenaHelp);
end

function RoleEmbattleUINew:OnClickCopyTeamBtn()
	self.objActor_Node:SetActive(false);
	local _callBack = function()
		self.objActor_Node:SetActive(true);
	end
...
function RoleEmbattleUINew:OnClickCloseBtn()
	if self.iGameState == -1 then
...
function RoleEmbattleUINew:MenPaiSingn()
...
function RoleEmbattleUINew:SetCallBack(callback)
	self.callback = callback;
end

function RoleEmbattleUINew:GetHelpData()
	if self.iGameState == -1 then
...
function RoleEmbattleUINew:GetTeamRoles()
...
function RoleEmbattleUINew:RemoveCantSelectRole(auiTeamList)
...
-- 
function RoleEmbattleUINew:CheckRoleEnterEvilBattle(roleID)
	if roleID == RoleDataManager:GetInstance():GetMainRoleID() then
		return true
	end
...
function RoleEmbattleUINew:RemoveClanRoleFromEmbattle(clanBaseID)
