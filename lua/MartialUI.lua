...
function MartialUI:IsBattleAIUIShow()
	return self.AIUI and self.AIUI.activeSelf
end
function MartialUI:InitBattleAIUI()
...
function MartialUI:Init(type, CharacterUI)
	...
	self.objDetail_template = self:FindChild(self.right_box, "SC_content/Viewport/template")
	...
	for i = 1, EMBATTLE_MARTIAL_MAX do
		local itemClass
		if self.parentType == 'Character' or self.parentType == 'NpcConsult' then
	...
	if self:IsShowHelpButton() then
		...
		if self.objHelpState then
			self.objHelpState:SetActive(true)
...
function MartialUI:IsShowHelpButton()
...
-- 显示tips
function MartialUI:ShowHelpValueTips()
...
function MartialUI:RefreshUI()
	self:UpdateMartialByRole()
end

function MartialUI:Update()
...
function MartialUI:InitOtherDataByType()
	if self.type == 'Clan' or self.type == 'NpcConsult' then		-- 门派学武界面
...
function MartialUI:UpdateMartialByRole(role)
...
function MartialUI:EmbattleMartialPage(isAdd)
...
function MartialUI:SortEmbattleMartial(info)
...
-- 服务器下行：更新全部出战配置
function MartialUI:UpdateEmbattleMartial(info)
...
-- [1] 更新出战配置中的一个武学（技能）
function MartialUI:UpdateSkillByMartial(itemIconClass, aInfo)
...
function MartialUI:OnClick_Embattle_Skill(iconData, dynData)
...
-- 玩家操作：将武学上阵/斜下
function MartialUI:OnClick_Embattle_Submit(aInfo, index)
...
function MartialUI:ParseMiEmbattle( iconData, miTypeData)
	-- [2-1] 普通技能，只显示第一个技能
...
function MartialUI:ParseEvoEmbattle( iconData, effectArray)
...
function MartialUI:ClearToggle()
	-- 清理左侧已有的武学列表
...
function MartialUI:ClearMartialItemTips()
	self:RemoveAllChildToPool(self.objDetail_content.transform)
end

-- 门派发过来的武学列表，只有静态 ID
function MartialUI:UpdateMartial(martial_list, isTypeID)
	self:ClearToggle()
	...
	if next(martial_list) then
		...
		for k, v in pairs(martial_list) do
			...
			if typeData.KFFeature ~= KFFeatureType.KFFT_AustrianMartialArts then
				self.iOwnMartialNum = self.iOwnMartialNum + 1
			end
		end
	...
	if self.type == 'Clan' or self.type == 'NpcConsult' then		-- 门派学武界面
		...
		self.objMartial_num.gameObject:SetActive(true)
		self.Martial_num.text = string.format("%d/%d",self.iOwnMartialNum, maxNum)		-- 武学数量
		self.Martial_num.gameObject:SetActive(true);
...
function MartialUI:UpdateDetail(dynData, typeData)
...
function MartialUI:UpdateStrongMark(level)
...
-- 更新右侧 / 标题，标签，系别，门派
function MartialUI:UpdateTitle(typeData)
...
-- 更新右侧 / 等级，经验条
function MartialUI:UpdateExp(dynData, typeData)
	if GetGameState() == -1 then
		self.exp_box:SetActive(false);
		return
	end
	...
	self.comLevel_TMP.text = string.format("%d/%d级", dynData['uiLevel'], dynData['uiMaxLevel'])
end

-- 更新右侧 / 技能图标
function MartialUI:UpdateIcon(dynData, typeData)
...
function MartialUI:ReturnUIClass()
...
function MartialUI:ParseMiToSkill( miTypeData, isLock, UnlockLvl, isCombo )
...
-- 处理普通技能的 UI 更新
function MartialUI:AddSkillNormal(skillID, isLock, unlockLvl, isCombo)
	local data = {
		['skillID'] = skillID,
		['isCombo'] = isCombo,
		['unlockLvl'] = unlockLvl,
...
-- 处理技能进化的 UI 更新
function MartialUI:AddSkillEvolution(effectArray, condID, isLock, unlockLvl)
...
function MartialUI:UpdateBtnForgottenMartial(dynData, typeData)
	if GetGameState() == -1 then
...
function MartialUI:UpdateBtnEquipMartial(dynData, typeData)
...
function MartialUI:IsEmbattleMartial(id)
	local martialList = globalDataPool:getData("EmbattleMartial")
	local info = martialList[self.selectRole]
...
function MartialUI:CheckMainRole_WANGYOUCAO()
...
function MartialUI:CheckTeammate_WANGYOUCAO(uiRoleUID)
...
-- 遗忘武学
function MartialUI:ForgottenMartial()
	--todo: 等级以及忘忧草判断
	--提示框
...
function MartialUI:NetGameCmdSend()
	local uiRoleUID = self.dynData.uiRoleUID
...
function MartialUI:AddSkillIcon(iconData, dynData, martialBaseID)
	local itemClass
	if self.parentType == 'Character' or self.parentType == 'NpcConsult' then
		--dprint("It's in CharacterUI")
		itemClass = LuaClassFactory:GetInstance():GetUIClass(LUA_CLASS_TYPE.SkillIconUIS,self.skill_box.transform)
	else
	 	itemClass = LuaClassFactory:GetInstance():GetUIClass(LUA_CLASS_TYPE.SkillIconUI,self.skill_box.transform)
	end
	-- if self.parentType == 'Character' then
	-- 	itemClass.transform.localScale= DRCSRef.Vec3(2/3,2/3,2/3)
	-- 	-- local rectItemClass =  itemClass._gameObject:GetComponent("RectTransform")
	...
	elseif  self.parentType == 'NpcConsult' then
			itemClass.parentType = 'NpcConsult'
...
-- 更新右侧 / 技能条目描述
function MartialUI:UpdateEntry(dynData, typeData)
...
function MartialUI:ShowAI(show)
	if self.btnAI then
		self.btnAI:SetActive(show)
	end
end

function MartialUI:UpdateStratergy()
...
function MartialUI:UpdateMartialNum()
	if self.type == 'Clan' or self.type == 'NpcConsult' then		-- 门派学武界面
        local curNum = RoleDataManager:GetInstance():GetRoleMartialNum()
		local maxNum = RoleDataManager:GetInstance():GetMartialNumMax()
		self.objMartial_num.gameObject:SetActive(true)
