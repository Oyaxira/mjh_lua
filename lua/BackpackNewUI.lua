BackpackNewUI = class("BackpackNewUI", BaseWindow)
...
--     }
-- }
function BackpackNewUI:ctor(kConfig)
    -- 必须参数: 绑定的ui节点, 背包行列数
...
function BackpackNewUI:SetSortButton(bShow)
...
function BackpackNewUI:_SetSortButtonVisible(bVisible)
    if self.objSortButton then
        self.objSortButton:SetActive(bVisible)
    end
end

function BackpackNewUI:InitSortNode()
    self.comRankToggles = {}
    for i = 1,7 do
        local iIndex = i
...
function BackpackNewUI:OnClickSortToggle(comToggles,iIndex,bIsOn)
    if iIndex == 0 then
        if bIsOn then 
...
function BackpackNewUI:UpdateSortButton(itemTypeList)
...
function BackpackNewUI:CheckSrotRankAndType()
...
-- 设置脏标
function BackpackNewUI:SetDirtyFlag(eFlag, kExtraInfo)
    if not eFlag then
        return
    end
...
-- 获取脏标信息
function BackpackNewUI:GetDirtyFlagExInfo(eFlag)
...
-- 清除脏标信息
function BackpackNewUI:ClearDirtyFlagExInfo(eFlag)
    if not (eFlag and self.kDirtyFlagExInfo) then
        return
    end
...
-- 清除脏标
function BackpackNewUI:ClearDirtyFlag(eFlag)
    if not eFlag then
        return
    end
...
-- 处理脏标
function BackpackNewUI:DealDirtyFlag(eFlag)
    if not eFlag then
        return
    end
    -- 刷新整个背包显示(静态数据)
...
-- 移除ui所有事件
function BackpackNewUI:RemoveAllRegisterListener()
...
-- 注册背包关心的事件监听
function BackpackNewUI:AddCareEventListener()
...
-- 根据不同的数据, 调用不同的显示接口
function BackpackNewUI:ShowPackByDataType()
...
-- 始化导航栏
function BackpackNewUI:InitNavBar()
    if not self.kCacheConfig then
        return
    end
    ...
    RemoveAllChildren(self.objNavBox)
    local objClone = nil
    local togFirst = nil
    for index = 1, #self.navIndex2Type do
        ...
        if navData and ((navData.types and #navData.types > 0) or navData.paging) then
            ...
            objClone = CloneObj(self.objNavBoxTemplate, transNavBox)
...
-- 重新初始化导航栏
function BackpackNewUI:ResetNavBar(navData)
...
-- 下拉框
function BackpackNewUI:UpdateDropDown(objDropdown, objText, strTab, itemTypeList)
...
function BackpackNewUI:BaseItemPassCheck(kBaseItem, kInstItem)
...
function BackpackNewUI:FilterBaseItems(akBaseItems, funcCond)
    local akFliterItems = {}
...
function BackpackNewUI:FilterInstItems(akInstItems, funcCond)
...
function BackpackNewUI:GetSCVShowCount(iRealCount)
    if not self.kCacheConfig then
        return 0
    end
...
-- 通过动态id数组转化为动态物品数据数组
function BackpackNewUI:GenInstItemListByInstIDList(aiIDs, akOutItems)
...
-- 通过静态id数组转化为静态物品数据数组
function BackpackNewUI:GenBaseItemListByBaseIDList(aiIDs, akOutItems)
    if not (aiIDs and akOutItems) then
        return
    end
    local kTBItem = TableDataManager:GetInstance():GetTable("Item")
...
-- 获取主角背包物品id列表
function BackpackNewUI:GetMainRoleItemInstDatas()
...
-- 使用动态数据更新滚动栏
function BackpackNewUI:UpdateNodeByInstItem(transform, index)
...
-- 使用静态数据更新滚动栏
function BackpackNewUI:UpdateNodeByBaseItem(transform, index)
...
-- 滚动栏更新接口
function BackpackNewUI:LoopScvUpdateFunc(transform, index)
...
-- 处理背包回调集合
function BackpackNewUI:SetBackPackFuncSet(kFuncSet)
...
-- 重置物品动态数据
function BackpackNewUI:ResetInstItemData(akInstItemArray)
...
-- 重置物品静态数据
function BackpackNewUI:ResetBaseItemData(akBaseItemArray)
...
function BackpackNewUI:SetPackByInstDataArray()
...
-- 真正的设置物品面板数据
-- !注意, 高消耗! 原则上外部只能设置数据与脏标, 不允许外部直接调用Set接口
function BackpackNewUI:SetPackByBaseDataArray()
    if not self.akBaseItemArray then
        return
    end
...
-- 刷新当前数据
function BackpackNewUI:RefreshPackData()
    self.comLoopScroll:RefreshCells()
end

-- 刷新并重置当前数据
function BackpackNewUI:RefreshPackAndResetData()
...
function BackpackNewUI:ShowPackByInstDataArray(akInstItems, kItemNumsMap, funcSet)
    if akInstItems then
...
function BackpackNewUI:ShowPackByBaseDataArray(akBaseItems, kItemNumsMap, funcSet)
    if akBaseItems then
...
function BackpackNewUI:ShowPackByInstIDArray(aiInstIDs, kItemNumsMap, funcSet)
...
function BackpackNewUI:ShowPackByBaseIDArray(akBaseIDs, kItemNumsMap, funcSet)
...
function BackpackNewUI:ShowAutoUpdateMainRolePack(bRegisterAutoUpdate, funcSet)
    -- 如果需要自动更新, 注册事件自动刷新数据
...
-- 刷新当前显示的物品数据
function BackpackNewUI:ShowRefreshPack()
    self:SetDirtyFlag(PackDirtyFlag.UPDATE_REFRESH)
end

-- 重置并刷新当前显示的动态物品数据, kItemNumsMap若无可不提供
function BackpackNewUI:ShowRefreshAndResetPackByInstItems(akInstItems, kItemNumsMap)
...
-- 重置并刷新当前显示的动态物品数据, kItemNumsMap若无可不提供
function BackpackNewUI:ShowRefreshAndResetPackByBaseItems(akBaseItems, kItemNumsMap)
...
-- 重置并刷新当前显示的动态物品数据, kItemNumsMap若无可不提供
function BackpackNewUI:ShowRefreshAndResetPackByInstIDArray(aiIDs, kItemNumsMap)
    local akInstItems = {}
...
-- 重置并刷新当前显示的动态物品数据, kItemNumsMap若无可不提供
function BackpackNewUI:ShowRefreshAndResetPackByBaseIDArray(aiIDs, kItemNumsMap)
    local akBaseItems = {}
...
function BackpackNewUI:PickItemByID(iID, iPickNum, bCover)
    if not iID then
        return
    end 
...
-- 通过id数组将一组物品添加到选中列表
function BackpackNewUI:PickItemByIDArray(aiIDs, aiNums, bCover)
    if not aiIDs then
        return
    end
    aiNums = aiNums or {}
...
-- 通过id将一个物品从选中列表移除
function BackpackNewUI:UnPickItemByID(iID)
...
-- 移除整个选中列表
function BackpackNewUI:UnPickAllItems()
    self.kPickedInfo = {['count'] = 0}
    -- set dirty flag
    self:SetDirtyFlag(PackDirtyFlag.UPDATE_REFRESH)
end
...
-- 获取所有选中的物品id
function BackpackNewUI:GetPickedItemIDArray()
    local list = {}
    local count = {}
...
function BackpackNewUI:GetPickedItemCount(iID)
...
-- 某个id对应的物品是否选中?
function BackpackNewUI:IsItemPicked(iID)
    return self:GetPickedItemCount(iID) > 0
end

-- 是否有物品被选中?
function BackpackNewUI:HasItemPicked()
...
-- 获取背包容量
function BackpackNewUI:GetPackSize()
    if self.akInstItemArray then
...
-- 将导航栏重置到第一个标签
function BackpackNewUI:Reset2FirstTap()
...
function BackpackNewUI:GetCurShowItemsArray()
    if self.akInstItemArrayToSet then
...
function BackpackNewUI:OnDestroy()
