SkillIconUINewS = class("SkillIconUINewS", BaseWindow)

function SkillIconUINewS:Create(kParent)
	local obj = LoadPrefabAndInit("Game/SkillIconUI1080",kParent,true)
	if obj then
		self:SetGameObject(obj)
	end
end

function SkillIconUINewS:Init()
...
function SkillIconUINewS:OnEnable()
...
function SkillIconUINewS:OnDisable()
    if not IsValidObj(self._gameObject) then
        return
    end
    if not self.canvasGroup then
...
function SkillIconUINewS:InitButtonListener()
    ...
    local comUIAction = self:FindChildComponent(self._gameObject,"Icon","LuaUIPointAction")
...
function SkillIconUINewS:UpdateUI(skillID, isCombo, isLock, unlock_level, coldTime, martialBaseID)
    if skillID == nil then
        return
    end
...
-- 设置层数/等级
function SkillIconUINewS:SetLayer(value)
...
function SkillIconUINewS:SetTipsValue(martialData,iSkillID,btns,funcGetBtns,iBaseID)
...
function SkillIconUINewS:ToPoolClear()
...
-- TODO: 更新技能图标
function SkillIconUINewS:UpdateSkillIcon(skillID)
...
-- TODO: 更新锁定状态
function SkillIconUINewS:UpdateLockState(isLock, unlock_level)
...
function SkillIconUINewS:UpdateCombo(isCombo)
    if self.objCombo then
        self.objCombo:SetActive(isCombo or false)
    end
end

function SkillIconUINewS:UpdateColdTime(coldTime, martialBaseID)
