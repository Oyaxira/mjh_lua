...
local sFilePath = "G:/DreamRivakes/LuaSnapshot/"
function BeginSnaoshot()
	--collectgarbage("collect")
	mri.m_cMethods.DumpMemorySnapshot(sFilePath, "1-Before", -1)
end

function EndSnaoshot()
	--collectgarbage("collect")
	...
	dprint("Dump memory snapshot information finished! ")
end

function DiffSnaoshot(str1,str2)
	mri.m_cMethods.DumpMemorySnapshotComparedFile(sFilePath, "Compared", -1, sFilePath..str1, sFilePath..str2)
	dprint("Dump memory snapshot information finished! ")
end

function LuaCollectgarbage()
    dprint("old mb:",collectgarbage("count") / 1024)
    collectgarbage("collect")
    dprint("now mb:",collectgarbage("count") / 1024)
end

function LuaCount()
	local t = collectgarbage("count") / 1024
	SystemUICall:GetInstance():Toast('Lua count:'..t)
end
